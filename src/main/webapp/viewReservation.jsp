...
<head>
    ...
    <style>
        ...
        .search-container { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(15px); padding: 25px 35px; border-radius: 15px; width: 100%; max-width: 550px; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.15); border: 1px solid rgba(255,255,255,0.6); margin-bottom: 30px; display: flex; flex-direction: column; gap: 15px; }
        .search-container form { display: flex; gap: 10px; }
        .search-container input { flex: 1; padding: 12px 15px; border: 1px solid #ccc; border-radius: 8px; font-size: 15px; font-family: 'Poppins', sans-serif; transition: 0.3s; }
        .search-container input:focus { border-color: var(--secondary); outline: none; box-shadow: 0 0 8px rgba(10,147,150,0.2); }
        .search-container button { background: linear-gradient(135deg, var(--primary), var(--secondary)); color: white; border: none; padding: 0 25px; border-radius: 8px; font-size: 15px; font-weight: 600; cursor: pointer; transition: 0.3s; font-family: 'Poppins', sans-serif; }
        .search-container button:hover { box-shadow: 0 5px 15px rgba(0,95,115,0.3); transform: translateY(-2px); }

        .details-card { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(15px); padding: 35px; border-radius: 15px; width: 100%; max-width: 600px; box-shadow: 0 15px 40px rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.6); border-top: 6px solid var(--secondary); }
...
<main>
    <div class="search-container">
        ...
        <p style="margin: 0 0 15px; font-size: 13px; color: #666;">Enter <strong>Booking ID</strong> (e.g. 5) OR <strong>Phone Number</strong></p>
    ...
    <div class="details-card">
        ...
        <div class="detail-row">
            <span class="label"><i class="fa-solid fa-id-card"></i> NIC / Passport:</span>
            <span class="value"><%= details.get("nic") != null ? details.get("nic") : "N/A" %></span>
        </div>
        <div class="detail-row">
